...
const banner = (options) => ({
  slots: {
    ...
    close: "text-(--ui-bg) hover:bg-(--ui-bg)/10 focus-visible:bg-(--ui-bg)/10"
  },
  variants: {
    color: {
...
const error = {
  slots: {
    ...
    statusMessage: "mt-2 text-4xl sm:text-5xl font-bold text-(--ui-text-highlighted) text-balance",
...
const footerColumns = (options) => ({
  ...
  variants: {
    active: {
      ...
      false: {
        link: ["text-(--ui-text-muted) hover:text-(--ui-text)", options.theme.transitions && "transition-colors"]
      }
    }
  }
});
...
const pageAnchors = (options) => ({
  ...
  variants: {
    active: {
      ...
      false: {
        link: ["text-(--ui-text-muted) hover:text-(--ui-text) font-medium", options.theme.transitions && "transition-colors"],
        linkLeading: ["bg-(--ui-bg-elevated)/50 ring-(--ui-border-accented) text-(--ui-text-dimmed) group-hover:bg-(--ui-primary) group-hover:ring-(--ui-primary) group-hover:text-(--ui-bg)", options.theme.transitions && "transition"]
      }
    }
  }
});
...
const pageAside = {
  slots: {
    ...
    topBody: "bg-(--ui-bg) relative pointer-events-auto flex -mx-4 px-4",
...
const user = (options) => ({
  ...
  variants: {
    ...
    to: {
      true: {
        name: ["text-(--ui-text) peer-hover:text-(--ui-text-highlighted)", options.theme.transitions && "transition-colors"],
        description: ["peer-hover:text-(--ui-text-muted-toned)", options.theme.transitions && "transition-colors"],
...
const a = (options) => ({
  base: ["text-(--ui-primary) border-b border-transparent hover:border-(--ui-primary) font-medium focus-visible:outline-(--ui-primary) [&>code]:border-dashed hover:[&>code]:border-(--ui-primary) hover:[&>code]:text-(--ui-primary)", options.theme.transitions && "transition-colors [&>code]:transition-colors"]
});
...
const callout = (options) => ({
  slots: {
    base: ["group relative block px-4 py-3 rounded-[calc(var(--ui-radius)*1.5)] text-sm/6 my-5 last:mb-0 [&_code]:text-xs/5 [&_code]:bg-(--ui-bg) [&_pre]:bg-(--ui-bg) [&>div]:my-2.5 [&_ul]:my-2.5 [&_ol]:my-2.5 [&>*]:last:!mb-0 [&_ul]:ps-4.5 [&_ol]:ps-4.5 [&_li]:my-0", options.theme.transitions && "transition-colors"],
...
const card = (options) => ({
  slots: {
    base: ["group relative block p-4 sm:p-6 border border-(--ui-border) rounded-[calc(var(--ui-radius)*1.5)] bg-(--ui-bg)", options.theme.transitions && "transition-colors"],
    icon: "size-6 mb-2 block",
    title: "text-(--ui-text-highlighted) font-semibold",
    description: "text-[15px] text-(--ui-text-muted)",
    externalIcon: ["size-4 align-top absolute right-2 top-2 text-(--ui-text-dimmed) pointer-events-none", options.theme.transitions && "transition-colors"]
  },
  variants: {
    color: {
      ...Object.fromEntries((options.theme.colors || []).map((color) => [color, {
...
const code = (options) => ({
  ...
  variants: {
    color: {
      ...Object.fromEntries((options.theme.colors || []).map((color) => [color, `border border-(--ui-${color})/25 bg-(--ui-${color})/10 text-(--ui-${color})`])),
...
const codeGroup = (options) => ({
  slots: {
    ...
    trigger: ["relative inline-flex items-center gap-1.5 text-(--ui-text) data-[state=active]:text-(--ui-text-highlighted) hover:bg-(--ui-bg-elevated)/50 px-2 py-1.5 text-sm rounded-[calc(var(--ui-radius)*1.5)] disabled:cursor-not-allowed disabled:opacity-75 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-(--ui-primary) focus:outline-none", options.theme.transitions && "transition-colors"],
...
const collapsible = (options) => ({
  slots: {
    base: "",
    trigger: ["group relative rounded-[calc(var(--ui-radius)/2)] inline-flex items-center gap-1.5 text-(--ui-text-muted) hover:text-(--ui-text) text-sm focus-visible:ring-2 focus-visible:ring-(--ui-primary) focus:outline-none", options.theme.transitions && "transition-colors"],
...
const field = {
  slots: {
    ...
    required: "rounded-(--ui-radius) bg-(--ui-error)/10 text-(--ui-error) px-1.5 py-0.5",
...
const h2 = (options) => ({
  slots: {
    base: ["relative text-2xl text-(--ui-text-highlighted) font-bold mt-12 mb-6 scroll-mt-[calc(48px+45px+var(--ui-header-height))] lg:scroll-mt-[calc(48px+var(--ui-header-height))] [&>a]:focus-visible:outline-(--ui-primary) [&>a>code]:border-dashed hover:[&>a>code]:border-(--ui-primary) hover:[&>a>code]:text-(--ui-primary) [&>a>code]:text-xl/7 [&>a>code]:font-bold", options.theme.transitions && "[&>a>code]:transition-colors"],
    leading: ["absolute -ms-8 top-1 opacity-0 group-hover:opacity-100 group-focus:opacity-100 p-1 bg-(--ui-bg-elevated) hover:text-(--ui-primary) rounded-[calc(var(--ui-radius)*1.5)] hidden lg:flex text-(--ui-text-muted)", options.theme.transitions && "transition"],
...
const h3 = (options) => ({
  slots: {
    base: ["relative text-xl text-(--ui-text-highlighted) font-bold mt-8 mb-3 scroll-mt-[calc(32px+45px+var(--ui-header-height))] lg:scroll-mt-[calc(32px+var(--ui-header-height))] [&>a]:focus-visible:outline-(--ui-primary) [&>a>code]:border-dashed hover:[&>a>code]:border-(--ui-primary) hover:[&>a>code]:text-(--ui-primary) [&>a>code]:text-lg/6 [&>a>code]:font-bold", options.theme.transitions && "[&>a>code]:transition-colors"],
    leading: ["absolute -ms-8 top-0.5 opacity-0 group-hover:opacity-100 group-focus:opacity-100 p-1 bg-(--ui-bg-elevated) hover:text-(--ui-primary) rounded-[calc(var(--ui-radius)*1.5)] hidden lg:flex text-(--ui-text-muted)", options.theme.transitions && "transition"],
    leadingIcon: "size-4 shrink-0",
    link: "group lg:ps-2 lg:-ms-2"
  }
});
...
const contentNavigation = (options) => ({
  ...
  compoundVariants: [{
    ...
    variant: "pill",
    class: {
      link: ["hover:text-(--ui-text-highlighted) hover:before:bg-(--ui-bg-elevated)/50 data-[state=open]:text-(--ui-text-highlighted)", options.theme.transitions && "transition-colors before:transition-colors"],
...
const contentSurround = (options) => ({
  slots: {
    ...
    link: ["group block px-6 py-8 rounded-[calc(var(--ui-radius)*2)] border border-(--ui-border) hover:bg-(--ui-bg-elevated)/50 focus-visible:outline-(--ui-primary)", options.theme.transitions && "transition-colors"],
    linkLeading: ["inline-flex items-center rounded-full p-1.5 bg-(--ui-bg-elevated) group-hover:bg-(--ui-primary)/10 ring ring-(--ui-border-accented) mb-4 group-hover:ring-(--ui-primary)/50", options.theme.transitions && "transition"],
    linkLeadingIcon: ["size-5 shrink-0 text-(--ui-text-highlighted) group-hover:text-(--ui-primary)", options.theme.transitions && "transition-[color,translate]"],
...
function getTemplates(options, uiOptions, nuxt) {
  ...
  function writeThemeTemplate(theme2, path) {
    for (const component in theme2) {
      templates.push({
        filename: `ui-pro/${path ? path + "/" : ""}${scule.kebabCase(component)}.ts`,
        write: true,
        getContents: async () => {
  ...
  if (!!nuxt && (kit.hasNuxtModule("@nuxtjs/mdc") || options.mdc || (kit.hasNuxtModule("@nuxt/content") || options.content))) {
    hasProse = true;
    const path = "prose";
    ...
    templates.push({
      ...
      write: true,
      getContents: () => Object.keys(themeProse).map((component) => `export { default as ${component} } from './${scule.kebabCase(component)}'`).join("\n")
    });
  }
  ...
  templates.push({
    ...
    write: true,
    getContents: () => {
      let contents = Object.keys(theme$1).map((component) => `export { default as ${component} } from './${scule.kebabCase(component)}'`).join("\n");
      if (hasContent) {
        contents += "\n";
        contents += Object.keys(themeContent).map((component) => `export { default as ${component} } from './content/${scule.kebabCase(component)}'`).join("\n");
      }
      if (hasProse)
        contents += `
...
async function _getPkgName(dir) {
  ...
  const projectPkg = await pkgTypes.readPackageJSON(dir).catch(() => null);
  if (projectPkg?.name) {
