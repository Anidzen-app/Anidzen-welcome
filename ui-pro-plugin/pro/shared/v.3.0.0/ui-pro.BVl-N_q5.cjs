...
const header = {
  slots: {
    ...
    overlay: "lg:hidden",
    header:
      "px-4 sm:px-6 h-(--ui-header-height) shrink-0 flex items-center justify-between gap-1.5",
  ...
  compoundVariants: [
    {
      ...
      class: {
        root: "flex-row",
        content:
          "flex-row animate-[marquee_var(--duration)_linear_infinite] rtl:animate-[marquee-rtl_var(--duration)_linear_infinite] backface-hidden perspective-1000",
      },
    },
    {
      orientation: "horizontal",
    ...
    },
    {
      ...
      class: {
        root: "flex-col",
        content:
          "flex-col animate-[marquee-vertical_var(--duration)_linear_infinite] rtl:animate-[marquee-vertical-rtl_var(--duration)_linear_infinite] h-[fit-content] backface-hidden perspective-1000",
      },
    },
    {
      orientation: "vertical",
    ...
    templates.push({
      ...
      getContents: () =>
        Object.keys(themeProse)
          .map(
            (component) =>
              `export { default as ${component} } from './${scule.kebabCase(component)}'`,
          )
          .join("\n"),
    });
  }
    ...
    getContents: () => {
      ...
      if (hasContent) {
        ...
        contents += Object.keys(themeContent)
          .map(
            (component) =>
              `export { default as ${component} } from './content/${scule.kebabCase(component)}'`,
          )
          .join("\n");
      }
      if (hasProse)
...
async function validateLicense(opts) {
  ...
  const gitInfo =
    opts.key !== "oss"
      ? void 0
      : (await _getLocalGitInfo(opts.dir)) || _getGitEnv();
  const projectName = gitInfo
...
async function _getLocalGitRemote(dir) {
  try {
    ...
    const gitDir = await pkgTypes.findNearestFile(".git/config", {
      startingFrom: dir,
    });
...
function _getGitEnv() {
  const envInfo = {
    ...
    provider:
      ...
      (process.env.GITHUB_SERVER_URL ? "github" : void 0),
    // github
    // Owner
    owner:
...
async function _getPkgName(dir) {
  ...
  throw new Error(
    "You must provide a `name` in your `package.json` to activate Nuxt UI Pro.",
  );
}
