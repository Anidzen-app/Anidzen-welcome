...
export async function validateLicense(opts: {
    ...
    try {
        ...
        const statusType = Math.round(
            ((error as FetchError).status as number) / 100,
        );
        if (statusType === 4) {
            throw _createError(
                `Invalid \`${opts.theme.env}\` license key.\nPurchase Nuxt UI Pro at \`${opts.theme.link}\` to build your app in production.`,
            );
        }
...
async function _getLocalGitInfo(rootDir: string): Promise<GitInfo | undefined> {
    ...
    const gitUrlParse = (await import('git-url-parse' as string).then(
        (r) => r.default || r,
    )) as (input: string) => Record<string, string>;
...
async function _getLocalGitRemote(dir: string) {
    try {
        ...
        const gitDir = await findNearestFile('.git/config', {
            startingFrom: dir,
        });
